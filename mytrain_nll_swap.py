...
sys.path.append('/home/haesoo/UD/Diff-MedVQA/Vision_Encoder_Decoder_MDiffVQA/')
sys.path.append('/home/haesoo/UD/Diff-MedVQA/Vision_Encoder_Decoder_MDiffVQA/pycocoevalcap/')

from mymodels.swinbertcross import SwinBERTFinetuned

from Vision_Encoder_Decoder_MDiffVQA.mydatasets.mimic_dataset_swap import mimic_Dataset
...
if args.load_weights != None:
    ...
    args.load_weights = "/home/haesoo/UD/Diff-MedVQA/EXPERIMENTS/" + args.load_weights
print('hnm:', args.hnm)
print('train_set:', args.train_set)
print(30*'*')

EXP_DIR_PATH = "/home/haesoo/UD/Diff-MedVQA/EXPERIMENTS/" + args.exp_name
...
if torch.cuda.device_count() > 1 and gpu_ids:
    print(f"Let's use {torch.cuda.device_count()} GPUs! Using GPUs {gpu_ids}")
...
test_dataset = mimic_Dataset(
    transform=model.module.val_transform, 
...
train_dataset = mimic_Dataset(
    transform=model.module.train_transform, 
    tokenizer=model.module.tokenizer,
    processor=model.module.processor,
...
for epoch in range(epochs):
    ...
    with tqdm(iter(train_dataloader), desc="Epoch " + str(epoch), unit="batch") as tepoch:
        for steps, batch in enumerate(tepoch):
            ...
            swapped_answers_ids = batch['swapped_answers_ids'].to(device)
            swapped_answers_mask = batch['swapped_answers_mask'].to(device)
            swapped_labels = batch['swapped_labels'].to(device)
           

         
            decoder_out_original = model(questions_ids=questions_ids,
            ...
            loss_original = decoder_out_original['loss'].mean()


            swapped_pixel_values = torch.flip(pixel_values, dims=[1])
            
            decoder_out_swapped = model(questions_ids=questions_ids,
                                        questions_mask=questions_mask,
                                        answers_ids=swapped_answers_ids, 
                                        answers_mask=swapped_answers_mask, 
                                        images=swapped_pixel_values, 
                                        images_mask=images_mask,
                                        labels=swapped_labels) 
            
            loss_swapped = decoder_out_swapped['loss'].mean()

     
            total_loss = loss_original + loss_swapped
            ...
            if args.hnm:
                multiassign(dict_loss, ids, 
                            [total_loss.to('cpu').detach().numpy()]) # total_loss로 변경
            
            # statistics
            train_loss += total_loss.item() # total_loss로 변경

            tepoch.set_description(f'Train Epoch [{epoch}/{epochs-1}] Loss: {total_loss.item():.4f}')
    ...
    if args.hnm:
        ...
        with tqdm(iter(HNM_trainloader), desc="Epoch " + str(epoch), unit="batch") as tepoch:
            for steps, batch in enumerate(tepoch):
                ...
                # statistics
                train_hnm_loss += total_loss.item()

                tepoch.set_description(f'HNM Epoch [{epoch}/{epochs-1}] Loss: {total_loss.item():.4f}')
    ...
    with torch.no_grad():
        ...
        with tqdm(iter(test_dataloader), desc="Epoch " + str(epoch), unit="batch") as tepoch:
            for steps, batch in enumerate(tepoch):
                ...
                generated_answers, _ = model.module.generate(
                    pixel_values, images_mask=images_mask,
                    questions_ids=questions_ids,
                    questions_mask=questions_mask,
                    tokenizer=model.module.tokenizer,
    ...
    if best_bleu1 < evaluator.evaluation_report['Bleu_1']:
        ...
        torch.save(model.module.state_dict(), EXP_DIR_PATH + "/best_bleu1_" + str(epoch) + "_model.pt")
    ...
    if best_cider < evaluator.evaluation_report['CIDEr']:
        ...
        torch.save(model.module.state_dict(), EXP_DIR_PATH + "/best_cider_" + str(epoch) + "_model.pt")
    ...
    if best_meteor < evaluator.evaluation_report['METEOR']:
        ...
        torch.save(model.module.state_dict(), EXP_DIR_PATH + "/best_meteor_" + str(epoch) + "_model.pt")
    ...
    if best_rougel < evaluator.evaluation_report['ROUGE_L']:
        ...
        torch.save(model.module.state_dict(), EXP_DIR_PATH + "/best_rougel_" + str(epoch) + "_model.pt")
...
# Save Final weights
torch.save(model.module.state_dict(), EXP_DIR_PATH + "/last_model.pt")
